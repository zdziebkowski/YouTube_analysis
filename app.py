...
df_channel = pd.read_csv(r'C:\Users\wojci\PycharmProjects\YouTubeAPI\data\channel_stats.csv')
...
with ui.nav_panel("KPI's"):
    with ui.layout_columns():
        with ui.card():
            with ui.value_box():
                "Total subscribers"
                f"{total_subscribers:,}".replace(',', ' ')
            with ui.value_box():
                "Total views"
                f"{total_views:,}".replace(',', ' ')
        with ui.card():
            with ui.value_box():
                "Average views"
                f"{average_views:,.0f}".replace(',', ' ')
            with ui.value_box():
                "Engagement Rate"
                f"{engagement_rate:,.2f}%".replace(',', ' ')
...
with ui.nav_panel("Analysis"):
    with ui.layout_sidebar():
        ...
        def plot_top_performing_videos():
            ...
            fig.add_trace(
                go.Bar(x=top_videos['short_title'], y=top_videos['comments'], name='Comments',
        ...
        with ui.layout_columns():
            ...
            def plot_sponsor_impact():
                ...
                sponsor_impact = list_df.groupby('sponsor').agg(
